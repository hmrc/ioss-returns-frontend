...
import queries.{AllVatRatesFromCountryQuery, VatRateFromCountryQuery}
...
trait GetVatRates {
  ...
  protected def getAllVatRatesFromCountry(countryIndex: Index)
                                         (block: Seq[VatRateFromCountry] => Future[Result])
                                         (implicit request: DataRequest[AnyContent]): Future[Result] = {
    block(
      request.userAnswers
        .get(AllVatRatesFromCountryQuery(countryIndex))
        .getOrElse(Seq.empty)
    )
  }
}
