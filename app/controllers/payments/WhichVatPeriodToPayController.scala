...
class WhichVatPeriodToPayController @Inject()(
  ...
  def onPageLoad(waypoints: Waypoints): Action[AnyContent] = cc.authAndRequireData().async {
    implicit request => {
      val allPayments: Future[List[Payment]] = paymentsService.getUnpaidPayments(request.iossNumber)
      allPayments.flatMap { payments =>
        val paymentError = payments.exists(_.paymentStatus == PaymentStatus.Unknown)

        if (payments.isEmpty) {
          Future.successful(Ok(viewNoPayment(YourAccountPage.route(waypoints).url)))
        } else if (payments.size == 1) {
          makePayment(request.vrn, payments.head)
        } else {
          Future.successful(Ok(view(form, payments, paymentError = paymentError)))
        }
      }
    }
  }
