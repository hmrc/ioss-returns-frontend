  ...
  def onPageLoad(waypoints: Waypoints, iossNumber: String): Action[AnyContent] = cc.authAndGetRegistrationAndCheckBounced(iossNumber).async {
    implicit request =>
      ...
      prepareFinancialData.flatMap { pfd =>
        val payments = (pfd.duePayments ++ pfd.overduePayments).sortBy(p => (p.period.year, p.period.month))
      ...
      prepareFinancialData.flatMap { pfd =>
        ...
        if (payments.size == 1) {
          ...
          form.bindFromRequest().fold(
            formWithErrors =>
              BadRequest(view(formWithErrors, request.iossNumber, payments, paymentError, request.isIntermediary, request.companyName)).toFuture,
            value =>
