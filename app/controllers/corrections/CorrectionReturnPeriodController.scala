  ...
  def onPageLoad(waypoints: Waypoints, index: Index): Action[AnyContent] = cc.authAndGetDataAndCorrectionEligible().async {
    implicit request =>
      ...
      filteredFulfilledObligations.map { obligations =>
        ...
        Ok(view(preparedForm, waypoints, period, uncompletedCorrectionPeriods, index, request.isIntermediary, request.registrationWrapper.getCompanyName()))
        ...
        form.bindFromRequest().fold(
          formWithErrors => {
            Future.successful(BadRequest(
              view(formWithErrors, waypoints, period, correctionPeriod, index, request.isIntermediary, request.registrationWrapper.getCompanyName())
            ))
          },
