...
trait SalesFromCountryBaseController {
  ...
  protected def getCountryAndVatRate(waypoints: Waypoints, countryIndex: Index, vatRateIndex: Index)
    ...
    (for {
      country <- request.userAnswers.get(SoldToCountryPage(request.iossNumber, countryIndex))
  ...
  protected def getCountryVatRateAndNetSales(countryIndex: Index, vatRateIndex: Index)
    ...
    (for {
      country <- request.userAnswers.get(SoldToCountryPage(request.iossNumber, countryIndex))
      vatRate <- request.userAnswers.get(VatRatesFromCountryQuery(countryIndex, vatRateIndex))
      netSales <- request.userAnswers.get(SalesToCountryPage(request.iossNumber, countryIndex, vatRateIndex))
    } yield {
