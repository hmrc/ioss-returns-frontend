...
class RedirectServiceSpec extends SpecBase with BeforeAndAfterEach {
  ...
  private implicit val dataRequest: DataRequest[AnyContent] = DataRequest[AnyContent](FakeRequest(), enrolments, testCredentials, Some(vrn), iossNumber, companyName, registrationWrapper, None, completedUserAnswersWithCorrections)
