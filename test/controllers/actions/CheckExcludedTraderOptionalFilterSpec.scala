  ...
  ".filter" - {
    ...
    "must return None when trader is excluded but can complete returns up to and including their exclusion effective date" in {
      ...
      running(application) {
        ...
        val request = OptionalDataRequest(FakeRequest(), enrolments, testCredentials, Some(vrn), iossNumber, companyName, excludedRegistration, None, Some(completeUserAnswers))
        val controller = new Harness(period)
