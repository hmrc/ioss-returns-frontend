...
class CheckExcludedTraderOptionalFilterSpec extends SpecBase with MockitoSugar with BeforeAndAfterEach {
  ...
  ".filter" - {
    ...
    "must return None when trader is not excluded" in {
      ...
      running(application) {
        val request = OptionalDataRequest(FakeRequest(), testCredentials, Some(vrn), iossNumber, registrationWrapper, None, Some(completeUserAnswers))
    ...
    "must return None when trader is excluded but can complete returns up to and including their exclusion effective date" in {
      ...
      running(application) {
        ...
        val request = OptionalDataRequest(FakeRequest(), testCredentials, Some(vrn), iossNumber, excludedRegistration, None, Some(completeUserAnswers))
        val controller = new Harness(period)
