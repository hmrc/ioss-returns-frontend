...
class CheckExcludedTraderFilterSpec extends SpecBase with MockitoSugar with BeforeAndAfterEach {
  ...
  ".filter" - {
    ...
    "must return None when trader is not excluded" in {
      ...
      running(application) {
        val request = DataRequest(FakeRequest(), testCredentials, Some(vrn), iossNumber, registrationWrapper, None, completeUserAnswers)
    ...
    "must return None when trader is excluded but can complete returns up to and including their exclusion effective date" in {
      ...
      running(application) {
        ...
        val request = DataRequest(FakeRequest(), testCredentials, Some(vrn), iossNumber, excludedRegistration, None, completeUserAnswers)
        val controller = new Harness()
