...
class SubmittedReturnForPeriodControllerSpec extends SpecBase with BeforeAndAfterEach with PrivateMethodTester {
  ...
  "SubmittedReturnForPeriod Controller" - {
    Seq(
      "onPageLoad" -> ((waypoints, period) => routes.SubmittedReturnForPeriodController.onPageLoad(waypoints, period))/*,
      "onPageLoadForIossNumber" -> ((waypoints, period) => routes.SubmittedReturnForPeriodController.onPageLoadForIossNumber(waypoints, period, iossNumber))*/
    ).map { case (callName, onPageLoad) =>
      callName - {
        "must return OK and the correct view for a GET" - {
          ...
          "when there are corrections present" in {
            ...
            running(application) {
              ...
              val request = FakeRequest(GET, onPageLoad(waypoints, period).url)
          ...
          "when there are exclusions present and vat return due date has exceeded 3 years" - {
            "and nothing owed" in {
              ...
              running(application) {
                ...
                val request =
                  OptionalDataRequest(
                    FakeRequest(GET, onPageLoad(waypoints, exceededPeriod).url),
                    testCredentials,
                    vrn,
                    iossNumber,
                    registrationWrapper,
