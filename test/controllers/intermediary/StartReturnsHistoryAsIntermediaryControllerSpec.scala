...
class StartReturnsHistoryAsIntermediaryControllerSpec extends SpecBase with MockitoSugar with BeforeAndAfterEach {
  ...
  "StartReturnsHistoryAsIntermediary Controller" - {

    "must redirect to the submitted returns history page" in {
      ...
      running(application) {
        val request = FakeRequest(GET, routes.StartReturnsHistoryAsIntermediaryController.startReturnsHistoryAsIntermediary(waypoints, iossNumber).url)
